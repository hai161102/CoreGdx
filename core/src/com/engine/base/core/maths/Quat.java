...
public class Quat implements IJson, Serializable {
    ...
    /** Constructor, sets the four components of the Quat.
    ...
    /** Constructor, sets the Quat components from the given Quat.
     *
     * @param quat The Quat to copy. */
    ...
    /** Constructor, sets the Quat from the given axis vector and the angle around that axis in degrees.
     *
     * @param axis The axis
    ...
    /** Sets the components of the Quat
     * @param x The x-component
     * @param y The y-component
     * @param z The z-component
     * @param w The w-component
    ...
    /** Sets the Quat components from the given Quat.
     * @param Quat The Quat.
     * @return This Quat for chaining. */
    ...
    /** Sets the Quat components from the given axis and angle around that axis.
     *
     * @param axis The axis
     * @param angle The angle in degrees
     * @return This Quat for chaining. */
    ...
    /** @return a copy of this Quat */
    public Quat cpy () {
        return new Quat(this);
    }

    /** @return the euclidean length of the specified Quat */
    public final static float len (final float x, final float y, final float z, final float w) {
        return (float)Math.sqrt(x * x + y * y + z * z + w * w);
    }

    /** @return the euclidean length of this Quat */
    public float len () {
        return (float)Math.sqrt(x * x + y * y + z * z + w * w);
    }
    

    /** Sets the Quat to the given euler angles in degrees.
    ...
    /** Sets the Quat to the given euler angles in radians.
    ...
    /** Get the roll euler angle in radians, which is the rotation around the z axis. Requires that this Quat is normalized.
    ...
    /** Get the roll euler angle in degrees, which is the rotation around the z axis. Requires that this Quat is normalized.
     * @return the rotation around the z axis in degrees (between -180 and +180) */
    public float getRoll () {
        return getRollRad() * MathUtils.radiansToDegrees;
    }

    /** Get the pitch euler angle in radians, which is the rotation around the x axis. Requires that this Quat is normalized.
    ...
    /** Get the pitch euler angle in degrees, which is the rotation around the x axis. Requires that this Quat is normalized.
     * @return the rotation around the x axis in degrees (between -90 and +90) */
    public float getPitch () {
        return getPitchRad() * MathUtils.radiansToDegrees;
    }

    /** Get the yaw euler angle in radians, which is the rotation around the y axis. Requires that this Quat is normalized.
     * @return the rotation around the y axis in radians (between -PI and +PI) */
    public float getYawRad () {
        return getGimbalPole() == 0 ? MathUtils.atan2(2f * (y * w + x * z), 1f - 2f * (y * y + x * x)) : 0f;
    }

    /** Get the yaw euler angle in degrees, which is the rotation around the y axis. Requires that this Quat is normalized.
    ...
    /** @return the length of this Quat without square root */
    public float len2 () {
        return x * x + y * y + z * z + w * w;
    }

    /** Normalizes this Quat to unit length
    ...
    /** Transforms the given vector using this Quat
     *
    ...
    /** Multiplies this Quat with another one in the form of this = this * other
     *
     * @param other Quat to multiply with
     * @return This Quat for chaining */
    ...
    /** Multiplies this Quat with another one in the form of this = this * other
     *
     * @param x the x component of the other Quat to multiply with
     * @param y the y component of the other Quat to multiply with
     * @param z the z component of the other Quat to multiply with
     * @param w the w component of the other Quat to multiply with
     * @return This Quat for chaining */
    ...
    /** Multiplies this Quat with another one in the form of this = other * this
     *
     * @param other Quat to multiply with
     * @return This Quat for chaining */
    ...
    /** Add the x,y,z,w components of the passed in Quat to the ones of this Quat */
    ...
    // TODO : the Mat4 set(Quat) doesnt set the last row+col of the matrix to 0,0,0,1 so... that's why there is this
// method
    /** Fills a 4x4 matrix with the rotation matrix represented by this Quat.
     *
    ...
    /** Sets the Quat to an identity Quat
     * @return this Quat for chaining */
    public Quat idt () {
        return this.set(0, 0, 0, 1);
    }

    /** @return If this Quat is an identity Quat */
    ...
    /** Sets the Quat from the given matrix, optionally removing any scaling. */
    public Quat setFromMatrix (boolean normalizeAxes, Mat4 matrix) {
        return setFromAxes(normalizeAxes, matrix.val[Mat4.M00], matrix.val[Mat4.M01], matrix.val[Mat4.M02],
                matrix.val[Mat4.M10], matrix.val[Mat4.M11], matrix.val[Mat4.M12], matrix.val[Mat4.M20],
                matrix.val[Mat4.M21], matrix.val[Mat4.M22]);
    }

    /** Sets the Quat from the given rotation matrix, which must not contain scaling. */
    public Quat setFromMatrix (Mat4 matrix) {
        return setFromMatrix(false, matrix);
    }

    /** Sets the Quat from the given matrix, optionally removing any scaling. */
    public Quat setFromMatrix (boolean normalizeAxes, Mat3 matrix) {
        return setFromAxes(normalizeAxes, matrix.val[Mat3.M00], matrix.val[Mat3.M01], matrix.val[Mat3.M02],
                matrix.val[Mat3.M10], matrix.val[Mat3.M11], matrix.val[Mat3.M12], matrix.val[Mat3.M20],
                matrix.val[Mat3.M21], matrix.val[Mat3.M22]);
    }

    /** Sets the Quat from the given rotation matrix, which must not contain scaling. */
    public Quat setFromMatrix (Mat3 matrix) {
        return setFromMatrix(false, matrix);
    }

    /**
     * <p>
     * Sets the Quat from the given x-, y- and z-axis which have to be orthonormal.
     * </p>
     *
     * <p>
    ...
    /**
     * <p>
     * Sets the Quat from the given x-, y- and z-axis.
     * </p>
     *
     * <p>
     * Taken from Bones framework for JPCT, see http://www.aptalkarga.com/bones/ which in turn took it from Graphics Gem code at
    ...
    /** Set this Quat to the rotation between two vectors.
     ...
     * @return This Quat for chaining */
    public Quat setFromCross (final float x1, final float y1, final float z1, final float x2, final float y2,
                                    final float z2) {
        final float dot = MathUtils.clamp(Vec3.dot(x1, y1, z1, x2, y2, z2), -1f, 1f);
        final float angle = (float)Math.acos(dot);
        return setFromAxisRad(y1 * z2 - z1 * y2, z1 * x2 - x1 * z2, x1 * y2 - y1 * x2, angle);
    }

    /** Spherical linear interpolation between this Quat and the other Quat, based on the alpha value in the range
     * [0,1]. Taken from Bones framework for JPCT, see http://www.aptalkarga.com/bones/
     * @param end the end Quat
    ...
    public Quat slerp (Quat end, float alpha) {
        ...
        // Check if the angle between the 2 Quats was big enough to
        // warrant such calculations
        if ((1 - absDot) > 0.1) {// Get the angle between the 2 Quats,
        ...
        // Calculate the x, y, z and w values for the Quat by using a
        // special form of linear interpolation for Quats.
    ...
    /** Spherical linearly interpolates multiple Quats and stores the result in this Quat. Will not destroy the data
     * previously inside the elements of q. result = (q_1^w_1)*(q_2^w_2)* ... *(q_n^w_n) where w_i=1/n.
     * @param q List of Quats
     * @return This Quat for chaining */
    ...
    /** Spherical linearly interpolates multiple Quats by the given weights and stores the result in this Quat. Will
    ...
    /** Calculates (this Quat)^alpha where alpha is a real number and stores the result in this Quat. See
     * http://en.wikipedia.org/wiki/Quat#Exponential.2C_logarithm.2C_and_power
...
// value
    ...
    /** Get the dot product between the two Quats (commutative).
     * @param x1 the x component of the first Quat
     * @param y1 the y component of the first Quat
     * @param z1 the z component of the first Quat
     * @param w1 the w component of the first Quat
     * @param x2 the x component of the second Quat
     * @param y2 the y component of the second Quat
     * @param z2 the z component of the second Quat
     * @param w2 the w component of the second Quat
     * @return the dot product between the first and second Quat. */
    ...
    /** Get the dot product between this and the other Quat (commutative).
     * @param other the other Quat.
     * @return the dot product of this and the other Quat. */
    public float dot (final Quat other) {
        return this.x * other.x + this.y * other.y + this.z * other.z + this.w * other.w;
    }

    /** Get the dot product between this and the other Quat (commutative).
     * @param x the x component of the other Quat
     * @param y the y component of the other Quat
     * @param z the z component of the other Quat
     * @param w the w component of the other Quat
     * @return the dot product of this and the other Quat. */
    public float dot (final float x, final float y, final float z, final float w) {
        return this.x * x + this.y * y + this.z * z + this.w * w;
    }

    /** Multiplies the components of this Quat with the given scalar.
     ...
     * </p>
     * This will normalize this Quat if needed. The received axis is a unit vector. However, if this is an identity
     * Quat (no rotation), then the length of the axis may be zero.
     *
     ...
     * @see <a href="http://www.euclideanspace.com/maths/geometry/rotations/conversions/QuatToAngle">calculation</a> */
    ...
    public float getAxisAngleRad (Vec3 axis) {
        if (this.w > 1) this.nor(); // if w>1 acos and sqrt will produce errors, this cant happen if Quat is normalised
        float angle = (float)(2.0 * Math.acos(this.w));
        double s = Math.sqrt(1 - this.w * this.w); // assuming Quat normalised then w is less than 1, so term always positive.
    ...
    /** Get the angle in radians of the rotation this Quat represents. Does not normalize the Quat. Use
    ...
    /** Get the angle in degrees of the rotation this Quat represents. Use {@link #getAxisAngle(Vec3)} to get both the
     * axis and the angle of this rotation. Use {@link #getAngleAround(Vec3)} to get the angle around a specific axis.
     ...
     * </p>
     * The swing and twist rotation can be used to reconstruct the original Quat: this = swing * twist
     *
    ...
    public void readJson(JsonValue value) {
        ...
        this.z = value.getFloat("z", 0);
        this.w = value.getFloat("w", 0);
    }
