...
public class Vec3 implements IJson, Serializable, Vector<Vec3> {
    ...
    private final static Mat4 tmpMat = new Mat4();
     ...
     * @param matrix The matrix
     * @return This vector for chaining */
    public Vec3 mul (final Mat4 matrix) {
        final float l_mat[] = matrix.val;
        return this.set(x * l_mat[Mat4.M00] + y * l_mat[Mat4.M01] + z * l_mat[Mat4.M02] + l_mat[Mat4.M03],
                x * l_mat[Mat4.M10] + y * l_mat[Mat4.M11] + z * l_mat[Mat4.M12] + l_mat[Mat4.M13],
                x * l_mat[Mat4.M20] + y * l_mat[Mat4.M21] + z * l_mat[Mat4.M22] + l_mat[Mat4.M23]);
    }

    /** Multiplies the vector by the transpose of the given matrix, assuming the fourth (w) component of the vector is 1.
     * @param matrix The matrix
     * @return This vector for chaining */
    public Vec3 traMul (final Mat4 matrix) {
        final float l_mat[] = matrix.val;
        return this.set(x * l_mat[Mat4.M00] + y * l_mat[Mat4.M10] + z * l_mat[Mat4.M20] + l_mat[Mat4.M30],
                x * l_mat[Mat4.M01] + y * l_mat[Mat4.M11] + z * l_mat[Mat4.M21] + l_mat[Mat4.M31],
                x * l_mat[Mat4.M02] + y * l_mat[Mat4.M12] + z * l_mat[Mat4.M22] + l_mat[Mat4.M32]);
    }

    /** Left-multiplies the vector by the given matrix.
     * @param matrix The matrix
     * @return This vector for chaining */
    public Vec3 mul (Mat3 matrix) {
        final float l_mat[] = matrix.val;
        return set(x * l_mat[Mat3.M00] + y * l_mat[Mat3.M01] + z * l_mat[Mat3.M02],
                x * l_mat[Mat3.M10] + y * l_mat[Mat3.M11] + z * l_mat[Mat3.M12],
                x * l_mat[Mat3.M20] + y * l_mat[Mat3.M21] + z * l_mat[Mat3.M22]);
    }

    /** Multiplies the vector by the transpose of the given matrix.
     * @param matrix The matrix
     * @return This vector for chaining */
    public Vec3 traMul (Mat3 matrix) {
        final float l_mat[] = matrix.val;
        return set(x * l_mat[Mat3.M00] + y * l_mat[Mat3.M10] + z * l_mat[Mat3.M20],
                x * l_mat[Mat3.M01] + y * l_mat[Mat3.M11] + z * l_mat[Mat3.M21],
                x * l_mat[Mat3.M02] + y * l_mat[Mat3.M12] + z * l_mat[Mat3.M22]);
    }

    /** Multiplies the vector by the given {@link Quat}.
     * @return This vector for chaining */
    public Vec3 mul (final Quat quat) {
     ...
     * @param matrix The matrix.
     * @return This vector for chaining */
    public Vec3 prj (final Mat4 matrix) {
        final float l_mat[] = matrix.val;
        final float l_w = 1f / (x * l_mat[Mat4.M30] + y * l_mat[Mat4.M31] + z * l_mat[Mat4.M32] + l_mat[Mat4.M33]);
        return this.set((x * l_mat[Mat4.M00] + y * l_mat[Mat4.M01] + z * l_mat[Mat4.M02] + l_mat[Mat4.M03]) * l_w,
     ...
     * @param matrix The matrix
     * @return This vector for chaining */
    public Vec3 rot (final Mat4 matrix) {
        final float l_mat[] = matrix.val;
        return this.set(x * l_mat[Mat4.M00] + y * l_mat[Mat4.M01] + z * l_mat[Mat4.M02],
                x * l_mat[Mat4.M10] + y * l_mat[Mat4.M11] + z * l_mat[Mat4.M12],
                x * l_mat[Mat4.M20] + y * l_mat[Mat4.M21] + z * l_mat[Mat4.M22]);
    }

    /** Multiplies this vector by the transpose of the first three columns of the matrix. Note: only works for translation and
     * rotation, does not work for scaling. For those, use {@link #rot(Mat4)} with {@link Mat4#inv()}.
     * @param matrix The transformation matrix
     * @return The vector for chaining */
    public Vec3 unrotate (final Mat4 matrix) {
        final float l_mat[] = matrix.val;
        return this.set(x * l_mat[Mat4.M00] + y * l_mat[Mat4.M10] + z * l_mat[Mat4.M20],
                x * l_mat[Mat4.M01] + y * l_mat[Mat4.M11] + z * l_mat[Mat4.M21],
                x * l_mat[Mat4.M02] + y * l_mat[Mat4.M12] + z * l_mat[Mat4.M22]);
    }
     ...
     * scaling. For those, use {@link #mul(Mat4)} with {@link Mat4#inv()}.
     * @param matrix The transformation matrix
     * @return The vector for chaining */
    public Vec3 untransform (final Mat4 matrix) {
        final float l_mat[] = matrix.val;
        x -= l_mat[Mat4.M03];
        y -= l_mat[Mat4.M03];
        z -= l_mat[Mat4.M03];
     ...
     * @return This vector for chaining */
    public Vec3 rotate (float degrees, float axisX, float axisY, float axisZ) {
     ...
     * @param axisZ the z-component of the axis
     * @return This vector for chaining */
    public Vec3 rotateRad (float radians, float axisX, float axisY, float axisZ) {
     ...
     * @param radians the angle in radians
     * @return This vector for chaining */
    public Vec3 rotateRad (final Vec3 axis, float radians) {
    ...
    public Vec3 fromString (String v) {
        ...
        throw new GdxRuntimeException("Malformed Vec3: " + v);
    }
    ...
    @Override
    public void readJson(JsonValue position) {
        this.x = position.getFloat("x", 0);
        this.y = position.getFloat("y", 0);
        this.z = position.getFloat("z", 0);
    }
